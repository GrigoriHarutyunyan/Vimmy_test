        ...
        @if (Route::has('login'))
            <div style="display: flex; flex-direction: row-reverse; justify-content: space-evenly;">
                @auth
                    ...
                    <a href="{{route('welcome')}}" class="text-sm text-gray-700 dark:text-gray-500 underline"> Назад</a>
                @else
            ...
            @foreach($userPosts as $userPost)
                ...
                @else
                    @foreach($userPost->posts as $post)
                        ...
                        @if(auth()->user() != null && auth()->user()->id == $userPost->id)
                            <div style="display: flex;justify-content:flex-end;">
                                <form action="{{route('post.destroy', ['id' => $post->id])}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button style="padding-right:15px;">Удалить</button>
                                </form>
